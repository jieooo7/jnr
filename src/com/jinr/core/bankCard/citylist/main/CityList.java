package com.jinr.core.bankCard.citylist.main;
...
import com.jinr.core.bankCard.citylist.db.DBManager;
...
//		initOverlay();
	...
	private ArrayList<CityModel> getSelectCityNames(String con) {
		...
		if (m.find() && m.group(0).equals(con)) {
			sqlString = "SELECT * FROM T_city WHERE ZoneName LIKE " + "\""
					+ "%" +con + "%" + "\"" + " ORDER BY Letter";
		} else {
			sqlString = "SELECT * FROM T_city WHERE NameShort LIKE " + "\""
					+ "%" + con + "%" + "\"" + " ORDER BY Letter";
		}
	...
	class CityListOnItemClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int pos,
			...
			intent.putExtra("cityNum",cityModel.getZongID());
